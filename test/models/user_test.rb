...
class UserTest < ActiveSupport::TestCase
  ...
  test "passwrod should be present (nonblank)" do
  ...
  test "user_name shoud not be too long" do
    @user.user_name = "a" * 51
    assert_not @user.valid?
  end
  ...
  test "feed should have the right post" do
    ...
    lana.user_images.each do |post_following|
      assert michael.feed.include?(post_following)
    end
    #自分自身の投稿を確認する
    michael.user_images.each do |post_self|
      assert michael.feed.include?(post_self)
    end
    #フォローしていないユーザーの投稿を確認する
    archer.user_images.each do |post_unfollowed|
